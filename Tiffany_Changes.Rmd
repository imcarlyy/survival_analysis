...
Let's begin: \# Challenge 1 (SOUMALYAS' PART) \# Challenge 1
...
## Challenge 5: Average Survival Curves: Comparison of Modeling Frameworks
...
Step 4: Fitting a Cox Proportional Hazards (CoxPH) Model We’ll now fit
the CoxPH model using sex as a predictor for survival. The CoxPH model
will treat competing events as censored data (which is its limitation).

```{r}
# Fit a Cox Proportional Hazards model using 'sex' as the predictor
cox_sex <- coxph(Surv(time, status) ~ sex, data = lung)
summary(cox_sex)
## I named it cox_m1 I think you can instead state something like we will be using the model created previously in challenge for
## The formula was coxph(Surv(time, status) ~ sex, data = lung) to remind everyone what it looks like
## instead you can just run summary(cox_m1)
...
ggsurvplot(survfit(Surv(time, status) ~ sex, data = lung),
           ...
           legend.labs = c("Male", "Female"),
           palette = c("blue", "red"))
## For this part I assigned the plot a variable which is plot_1 so i think u can just run that since both ben and I also used this plot
```
...
plot(cif, xlab = "Time (Days)", ylab = "Cumulative Incidence",
     ...
     col = c("blue", "red"))
legend("bottomright", legend = names(cif), col = c("blue", "red"), lty = 1) ##T.z: I am getting an error that says Error in (function (s, units = "user", cex = NULL, font = NULL, vfont = NULL,  :  plot.new has not been called yet
## I think this graph can be done in gg plot for prettier purposes if you want

```

```{r}
library(ggplot2)
ci1 <- data.frame(time = cif[[1]]$time, est = cif[[1]]$est, group = "Group 1")
ci2 <- data.frame(time = cif[[2]]$time, est = cif[[2]]$est, group = "Group 2")
ci_df <- rbind(ci1, ci2)
ggplot(ci_df, aes(x = time, y = est, color = group)) +
  geom_step() +
  labs(x = "Time (Days)", y = "Cumulative Incidence",
       title = "CRR Model: Cumulative Incidence by Sex") +
  theme_minimal()
```
